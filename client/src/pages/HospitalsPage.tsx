...
export const HospitalsPage: React.FC<HospitalsPageProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 pb-20 md:pb-8">
      ...
      <div className="glass-panel rounded-2xl p-4 mb-6 border border-emerald-300 dark:border-slate-800 space-y-4 bg-white/95 dark:bg-slate-900/90 shadow-md">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-emerald-50/60 dark:bg-slate-900 border border-emerald-300 dark:border-slate-700 rounded-xl text-sm text-emerald-950 dark:text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 font-bold shadow-xs"
            />
          </div>
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer transition-all border shrink-0 w-full sm:w-auto shadow-xs ${
              only247
                ? 'bg-red-50 text-red-700 dark:bg-red-500/20 dark:text-red-400 border-red-400 dark:border-red-500/50 font-bold'
                : 'bg-emerald-50 text-emerald-950 dark:bg-slate-900 dark:text-slate-300 border-emerald-300 dark:border-slate-700 hover:bg-emerald-100 font-bold'
            }`}
          >
            <ShieldAlert className="w-4 h-4 text-red-500" /> Chỉ hiện Cấp cứu 24/7
          </button>
        </div>

        {/* Specialty Pills: Mint Background with Emerald Border & Dark Emerald Text */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
          <span className="text-xs text-slate-600 dark:text-slate-400 font-bold flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer border shadow-2xs ${
                selectedSpecialty === spec
                  ...
                  : 'bg-emerald-50 text-emerald-950 dark:bg-slate-800 dark:text-slate-200 border-emerald-300 dark:border-slate-700 hover:bg-emerald-100 font-bold'
              }`}
            >
              <span className={selectedSpecialty === spec ? 'text-white font-bold' : 'text-emerald-950 dark:text-slate-200 font-bold'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {filteredHospitals.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-500 dark:text-slate-400 glass-panel rounded-2xl bg-white/95 dark:bg-slate-900 border border-emerald-200">
            <Clock className="w-10 h-10 mx-auto text-emerald-600 mb-2" />
            <p className="text-sm font-semibold text-slate-800">Không tìm thấy bệnh viện phù hợp với bộ lọc hiện tại.</p>
          </div>
        )}
      </div>
    </div>
